...
Function mcSetPivotTableFormat($mcPivotTable) { # Set pivotTable cosmetics and sheet name
    ...
    if ($null -ne $mcPivotTable[11]) { # $mcPivotTable[11] Set ColorScale
...
# ---------------------------------------------------------------------------------------
Write-Info "Import $($CompleteCSVFileName) to Excel."
...
if ($mcCurrentRow -ne 1) {
	[void]($Sheet1.Cells.Item($mcCurrentRow,1).entireRow).delete()  # Delete header on 2nd and later CSV.
}
$mcCurrentRow = $Sheet1.UsedRange.EntireRow.Count + 1

Write-Info "Customizing XLSX"
...
$PivotTable1 = $mcWorkbooks.PivotCaches().Create(1,"Sheet1!R1C1:R$($Sheet1.UsedRange.Rows.count)C$($Sheet1.UsedRange.Columns.count)",5)
...
$PivotTable2 = $mcWorkbooks.PivotCaches().Create(1,"Sheet1!R1C1:R$($Sheet1.UsedRange.Rows.count)C$($Sheet1.UsedRange.Columns.count)",5)
...
$PivotTable3 = $mcWorkbooks.PivotCaches().Create(1,"Sheet1!R1C1:R$($Sheet1.UsedRange.Rows.count)C$($Sheet1.UsedRange.Columns.count)",5)
...
$PivotTable4 = $mcWorkbooks.PivotCaches().Create(1,"Sheet1!R1C1:R$($Sheet1.UsedRange.Rows.count)C$($Sheet1.UsedRange.Columns.count)",5)
...
$PivotTable5 = $mcWorkbooks.PivotCaches().Create(1,"Sheet1!R1C1:R$($Sheet1.UsedRange.Rows.count)C$($Sheet1.UsedRange.Columns.count)",5)
...
$PivotTable6 = $mcWorkbooks.PivotCaches().Create(1,"Sheet1!R1C1:R$($Sheet1.UsedRange.Rows.count)C$($Sheet1.UsedRange.Columns.count)",5)
...
$PivotTable7 = $mcWorkbooks.PivotCaches().Create(1,"Sheet1!R1C1:R$($Sheet1.UsedRange.Rows.count)C$($Sheet1.UsedRange.Columns.count)",5)
...
foreach ($mcWorkSheet in $mcWorkbooks.Worksheets) {
	$mcWorkSheetNames.add($mcWorkSheet.Name) | Out-null
}
$mctmp = $mcWorkSheetNames.Sort() | Out-Null
For ($i=0; $i -lt $mcWorkSheetNames.Count-1; $i++) {
